...
// StonFi API client
const { StonApiClient } = _require("@ston-fi/api");
const stonApi = new StonApiClient();

// StonFi SDK (for swap execution -- may not be installed)
let dexFactory;
try {
  const stonfi = _require("@ston-fi/sdk");
...
// ---------------------------------------------------------------------------
// Asset cache (5-minute TTL)
// queryAssets text search doesn't work, so we cache getAssets() and filter.
// ---------------------------------------------------------------------------

let _assetCache = null;
let _assetCacheTime = 0;
const CACHE_TTL = 5 * 60 * 1000;

async function getCachedAssets() {
  if (_assetCache && Date.now() - _assetCacheTime < CACHE_TTL) return _assetCache;
  _assetCache = await stonApi.getAssets();
  _assetCacheTime = Date.now();
  return _assetCache;
}
