...
function parseAmount(amount, vault) {
  ...
  if (v === "usdt") return toStablecoin(Number(amount));
  return numToNano(Number(amount)); // NOT and native are 9 decimals
}
  ...
  execute: async (params) => {
    try {
      ...
      const increaseOpts = {
        ...
        leverage: numToNano(Number(params.leverage)),
      };
      if (params.stop_loss) increaseOpts.stopTriggerPrice = numToNano(Number(params.stop_loss));
      if (params.take_profit) increaseOpts.takeTriggerPrice = numToNano(Number(params.take_profit));
...
const stormClosePosition = {
  ...
  execute: async (params) => {
    try {
      ...
      let size;
      if (params.size) {
        size = numToNano(Number(params.size));
      } else {
  ...
  execute: async (params) => {
    try {
      ...
      const txParams = await sdk.removeMargin({
        ...
        traderAddress,
        direction,
        amount: numToNano(Number(params.amount)),
      });
    ...
    try {
      ...
      if (params.order_type === "stopLoss" || params.order_type === "takeProfit") {
        ...
        orderOpts = {
          ...
          direction,
          amount: numToNano(Number(params.amount)),
          trigerPrice: numToNano(Number(params.trigger_price)),
          expiration,
        };
      } else {
        ...
        orderOpts = {
          ...
          amount: parseAmount(params.amount, params.vault),
          leverage: numToNano(Number(params.leverage)),
          limitPrice: numToNano(Number(params.limit_price)),
          stopTriggerPrice: numToNano(Number(params.stop_trigger_price || "0")),
          takeTriggerPrice: numToNano(Number(params.take_trigger_price || "0")),
          expiration,
        };
        if (params.order_type === "stopLimit" && params.stop_price) {
          orderOpts.stopPrice = numToNano(Number(params.stop_price));
        }
      }
...
const stormUnstake = {
  ...
  execute: async (params) => {
    try {
      ...
      if (params.amount) unstakeOpts.amount = numToNano(Number(params.amount));
