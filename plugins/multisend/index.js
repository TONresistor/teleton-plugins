...
const { HighloadWalletV3 } = _require("@tonkite/highload-wallet-v3");
...
const SEQ_FILE = join(homedir(), ".teleton", "multisend-sequence.json");
...
async function getMultisendWallet() {
  const { keyPair } = await getWalletData();

  let sequence;
  try {
    const seqData = JSON.parse(readFileSync(SEQ_FILE, "utf-8"));
    sequence = HighloadWalletV3.restoreSequence(seqData.lastQueryId);
  } catch {
...
function saveSequence(sequence) {
  writeFileSync(SEQ_FILE, JSON.stringify({ lastQueryId: sequence.current() }));
}
...
const multisendInfo = {
  ...
  execute: async (_params, _context) => {
    try {
      const { wallet, sequence } = await getMultisendWallet();
      const client = await getClient();
      const balance = await client.getBalance(wallet.address);
      const state = await client.getContractState(wallet.address);
      const deployed = state.state === "active";

      // Read local sequence file
      let seqInfo = { lastQueryId: sequence.current(), hasNext: sequence.hasNext() };
      if (existsSync(SEQ_FILE)) {
        try {
          const seqData = JSON.parse(readFileSync(SEQ_FILE, "utf-8"));
          seqInfo.savedQueryId = seqData.lastQueryId;
        } catch { /* ignore */ }
      }
...
const multisendFund = {
  ...
  execute: async (params, _context) => {
    try {
      const { wallet: multisendWallet, keyPair } = await getMultisendWallet();
...
const multisendBatchTon = {
  ...
  execute: async (params, _context) => {
    try {
      ...
      const { wallet, keyPair, sequence } = await getMultisendWallet();
      const client = await getClient();
...
export const tools = [multisendInfo, multisendFund, multisendBatchTon, multisendBatchJetton, multisendStatus];
