...
import { getLatestActiveOrderForUser, getOrderByRef as loadOrderByRef, updateOrderStatus as setDbOrderStatus, upsertOrder as saveOrder, } from "./order-repository.js";
...
async function pollOrderInBackground(sdk, refId, chatId, messageId, activeChecks, lang) {
    ...
    try {
        while (Date.now() - startedAt < maxDurationMs) {
            ...
            if (status === "awaiting_payment") {
                if (Date.now() - lastProgressAt >= progressUpdateEveryMs) {
                    lastProgressAt = Date.now();
                    await updateText(lang === "en" ? `Checking payment for order ${refId}...` : `Проверяю оплату по заказу ${refId}...`);
                }
                await sleep(pollIntervalMs);
                continue;
            }
            const errorText = String(result?.error || result?.message || "unknown error");
            await updateText(lang === "en" ? `Failed to process order ${refId}: ${errorText}` : `Не удалось обработать заказ ${refId}: ${errorText}`);
            return;
        }
        await updateText(lang === "en"
...
export function createTools(sdk, activeChecks) {
    return [
        {
            ...
            async execute(params, context) {
                ...
                try {
                    ...
                    return {
                        ...
                        data: {
                            ref_id: refId,
                            status: "error",
                            message: resolveLang(sdk, params.lang) === "en"
                ...
                const deepLinkRaw = deepLinkRawFromApi ||
                ...
                setOrderStatus(sdk, refId, "pending", order, ttlMs);
        ...
        },
        {
            ...
            description: "Шаг 2/2. Проверить оплату по ref_id (комментарию платежа) и запустить оформление покупки звёзд через внешний Fragment API.\n" +
            ...
            async execute(params, context) {
                ...
                const inferredOrder = !explicitRefId && context.chatId && context.senderId
                ...
                if (!order) {
                    return {
                        success: false,
                        error: resolveLang(sdk, params.lang || order?.lang) === "en"
