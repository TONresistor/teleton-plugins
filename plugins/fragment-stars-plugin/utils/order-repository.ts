...
function ensureColumn(db: Database, tableName: string, columnName: string, columnSpec: string): void {
...
function mapOrderRow(row: any): OrderRecord | null {
  if (!row) {
    return null;
  }
  return {
...
export function getOrderByRef(db: Database, refId: string): OrderRecord | null {
  const row = db.prepare("SELECT * FROM stars_orders WHERE ref_id = ?").get(refId);
  return mapOrderRow(row);
}

export function getLatestActiveOrderForUser(
  db: Database,
  chatId: string,
  senderId: string,
): OrderRecord | null {
...
export function upsertOrder(db: Database, order: OrderRecord): void {
...
export function listCheckingOrders(db: Database): CheckingOrderRow[] {
  return db.prepare("SELECT ref_id, chat_id FROM stars_orders WHERE status = 'checking'").all() as CheckingOrderRow[];
}
