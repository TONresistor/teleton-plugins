...
export interface RuntimeSdk {
  ...
  ton: {
    ...
    getTransactions(address: string, limit?: number): Promise<Array<{ hash: string; amount?: string; from?: string; to?: string; comment?: string | null; date: string; secondsAgo: number }>>;
    verifyPayment(payload: {
      amount: number;
      memo: string;
      gameType: string;
      maxAgeMinutes: number;
    }): Promise<{ verified: boolean; error?: string; txHash?: string; playerWallet?: string }>;
    sendTON(address: string, amountTon: number, memo: string): Promise<{ txHash?: string; hash?: string }>;
  };
  log: {
